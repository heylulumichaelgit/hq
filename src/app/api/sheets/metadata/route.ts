...
export async function GET(request: NextRequest) {
  ...
  try {
    const metadata = await getSpreadsheetMetadata(authClient, spreadsheetId);
    return NextResponse.json(metadata);
  } catch (err) {
    console.error("Sheets metadata error:", err instanceof Error ? err.message : "Unknown error");
    return NextResponse.json({ error: "Failed to fetch spreadsheet metadata" }, { status: 500 });
  }
}
